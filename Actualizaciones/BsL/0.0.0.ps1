...
function GetRadioBorde {
    param($propiedades, $control)
    
    if ($propiedades.ContainsKey('circular')) {
        $valor = $propiedades['circular']
        if ($valor -match '(\d+)xp') {
...
function GetColor {
    param($propiedades, [string]$prop, $defecto)
    
    if ($propiedades.ContainsKey($prop)) {
        $colorStr = $propiedades[$prop]
        try {
...
function GetPadding {
    param($propiedades, [string]$prop, [int]$defecto)
    
    if ($propiedades.ContainsKey($prop)) {
        $valor = $propiedades[$prop]
        if ($valor -match '(\d+)') {
            return [int]$matches[1]
        }
    }
...
function GetString {
    param($propiedades, [string]$prop, [string]$defecto)
    
    if ($propiedades.ContainsKey($prop)) {
        return $propiedades[$prop]
    }
    return $defecto
}

function GetBoolean {
    param($propiedades, [string]$prop, [bool]$defecto)
    
    if ($propiedades.ContainsKey($prop)) {
        $valor = $propiedades[$prop].ToLower()
...
function GetFont {
    param($propiedades, $fuenteBase)
    
    $fuente = $fuenteBase
    $estilo = [System.Drawing.FontStyle]::Regular
    
    if ($propiedades.ContainsKey('negrita') -and (GetBoolean $propiedades 'negrita' $false)) {
        $estilo = $estilo -bor [System.Drawing.FontStyle]::Bold
    }
    if ($propiedades.ContainsKey('cursiva') -and (GetBoolean $propiedades 'cursiva' $false)) {
        $estilo = $estilo -bor [System.Drawing.FontStyle]::Italic
    }
    if ($propiedades.ContainsKey('subrayado') -and (GetBoolean $propiedades 'subrayado' $false)) {
        $estilo = $estilo -bor [System.Drawing.FontStyle]::Underline
    }
    
    $tamano = $fuente.Size
    if ($propiedades.ContainsKey('tamano')) {
        $tamanoStr = $propiedades['tamano']
...
$form.Text = "Mi Lenguaje de Programación - [Nuevo Programa.bsl]"
...
$titleLabel.Text = "  Mi Lenguaje de Programación - [Nuevo Programa.bsl]"
...
# Usar caracteres sin acentos para evitar problemas de encoding
$basico = $treeInstrucciones.Nodes.Add("BASICO")
$basico.Nodes.Add("var - Declarar variable")
$basico.Nodes.Add("if - Condicional")
$basico.Nodes.Add("while - Bucle")
$basico.Nodes.Add("for - Iteracion")

$funciones = $treeInstrucciones.Nodes.Add("FUNCIONES")
$funciones.Nodes.Add("function - Definir funcion")
...
$gui = $treeInstrucciones.Nodes.Add("INTERFAZ GRAFICA")
$gui.Nodes.Add("bsl frame - Crear ventana")
$gui.Nodes.Add("bsl etiqueta - Texto")
$gui.Nodes.Add("bsl boton - Boton")
...
# Función para actualizar números de línea - CORREGIDA
function ActualizarNumerosLinea {
    $lineNumbers.Controls.Clear()
    $lineCount = $editorTexto.Lines.Length
    for ($i = 1; $i -le $lineCount; $i++) {
        $lblNum = New-Object System.Windows.Forms.Label
        $lblNum.Text = $i.ToString()
        # CORREGIDO: Calcular la posición fuera del constructor
        $posX = 5
        $posY = ($i - 1) * 18 + 3
        $lblNum.Location = New-Object System.Drawing.Point($posX, $posY)
...
function AplicarEstilosControl($control, $linea) {
    $propiedades = ParsearEstilos $linea
    
    if ($propiedades.ContainsKey('colorfondo')) {
        $control.BackColor = GetColor $propiedades 'colorfondo' $control.BackColor
    }
    
    if ($propiedades.ContainsKey('color')) {
        $control.ForeColor = GetColor $propiedades 'color' $control.ForeColor
    }
    
    if ($propiedades.ContainsKey('tamano') -or 
        $propiedades.ContainsKey('negrita') -or 
        $propiedades.ContainsKey('cursiva') -or 
        $propiedades.ContainsKey('subrayado')) {
        $control.Font = GetFont $propiedades $control.Font
    }
...
function AplicarEstilosBoton($control, $linea) {
    $propiedades = AplicarEstilosControl $control $linea
    
    $control.FlatStyle = "Flat"
    $control.FlatAppearance.BorderSize = 0
    
    if ($propiedades.ContainsKey('borde')) {
        $grosor = GetPadding $propiedades 'borde' 0
        if ($grosor -gt 0) {
    ...
    if ($propiedades.ContainsKey('padding')) {
        $padding = GetPadding $propiedades 'padding' 0
        $control.Padding = New-Object System.Windows.Forms.Padding($padding)
    }
    
    $radio = GetRadioBorde $propiedades $control
    if ($radio -gt 0) {
        $control.Tag = $radio
    }
    
    if (GetBoolean $propiedades 'sombra' $false) {
...
function AplicarEstilosCajaTexto($control, $linea) {
    ...
    if (GetBoolean $propiedades 'password' $false) {
        $control.PasswordChar = '*'
    }
    
    if ($propiedades.ContainsKey('placeholder')) {
        $placeholder = GetString $propiedades 'placeholder' ''
    ...
    if ($propiedades.ContainsKey('alineacion')) {
        $alineacion = GetString $propiedades 'alineacion' 'izquierda'
    ...
    if ($propiedades.ContainsKey('borde')) {
        $grosor = GetPadding $propiedades 'borde' 1
        ...
        if ($codigo -match 'bsl frame titulo="([^"]*)" tamano="([^"]*)"') {
            ...
            foreach ($linea in $lineas) {
                ...
                elseif ($linea -match '^bsl lista titulo="([^"]*)" posicion="([^"]*)" tamano="([^"]*)"') {
                    ...
                    $propiedades = AplicarEstilosControl $listBox $linea
